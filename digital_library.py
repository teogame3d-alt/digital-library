"""Digital library domain model, persistence layer, and CLI demo.

The project stays intentionally compact because it is a portfolio baseline:
it shows object-oriented state changes, JSON persistence, and tests without
introducing a framework before the domain rules are clear.
"""
...
class Book:
    ...
    def to_dict(self) -> dict:
        """Serialize the book into JSON-friendly data."""
    ...
    def from_dict(data: dict) -> "Book":
        """Recreate a Book from saved JSON data."""
...
class Library:
    ...
    def add_book(self, book: Book) -> None:
        """Adds a book to the in-memory catalog.

        Persistence is explicit: the CLI calls save_to_file() only when the
        user decides to write the current state to JSON.
        """
    ...
    def snapshot(self) -> dict:
        """Return a structured summary useful for tests, reports, or UI layers."""

        total = len(self.books)
        available_count = sum(1 for book in self.books if book.available)
        borrowed_count = total - available_count
        return {
            "name": self.name,
            "total_books": total,
            ...
            "total_successful_borrows": self.total_successful_borrows,
            "categories": sorted({book.category for book in self.books}),
        }
...
# ==========================
def main():
    """Run the menu-driven CLI demo.

    The interactive layer is deliberately thin: it delegates business rules to
    Library and Reader so the tested domain objects remain reusable.
    """
    ...
    library.add_book(
        Book("The Little Prince", "Antoine de Saint-Exupery", 1943, category="Fiction")
    )
